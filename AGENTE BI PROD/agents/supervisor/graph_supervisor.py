...
def _build_state_summary(state: Dict[str, Any]) -> str:
    ...
    summary_lines = [
        ...
        f"sql_results_count: {len(sql_results)}",
        "",
        "=== RESUMEN SQL ===",
...
No hay resultados SQL aún.
        ...
        # 2. Plan válido, sin SQL → sql_agent
        HumanMessage(content="""
=== ESTADO ACTUAL ===
iteration_count: 1
last_agent: planner
...
No hay resultados SQL aún.
        """.strip()),
        AIMessage(content=json.dumps({
            "reasoning": "Plan válido con tareas pendientes. Se debe ejecutar SQL para obtener datos.",
            "next_agent": "sql_agent",
            "feedback_to_planner": None,
            "feedback_to_sql_agent": None
        }, ensure_ascii=False)),

        # 3. SQL ejecutado con éxito → analyst
        HumanMessage(content="""
=== ESTADO ACTUAL ===
iteration_count: 3
...
- success: 1 tarea(s)
  - tarea t1: Query ejecutada correctamente, 45 filas
        """.strip()),
        AIMessage(content=json.dumps({
            "reasoning": "SQL ejecutado con éxito. Se redacta respuesta final.",
            "next_agent": "analyst",
            "feedback_to_planner": None,
            "feedback_to_sql_agent": None
        }, ensure_ascii=False)),

        # 4. Respuesta lista → FINISH
        HumanMessage(content="""
=== ESTADO ACTUAL ===
iteration_count: 5
last_agent: analyst
...
def supervisor_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 5. Pre-dispatch absoluto: evitar que el LLM termine sin datos
    statuses = _extract_statuses(sql_results)

    # needs_clarification → terminar con pregunta al usuario
    if statuses["needs_clarification"] and not final_answer:
        questions = "\n".join(
    ...
    # unrecoverable → replanificar con feedback
    if statuses["unrecoverable"] and last_agent != "planner":
        feedback = "\n".join(
    ...
    # ============================================================
    # GUARDRAIL ABSOLUTO: Nunca terminar sin datos ejecutados
    # ============================================================
    has_data = (
        statuses["success"]
        or statuses["partial"]
        or state.get("forecast_results") is not None
        or state.get("research_findings") is not None
    )

    # Si no hay datos y el plan tiene tareas → SQL Agent (salvo que ya venga de SQL)
    if not has_data and plan.tasks and last_agent != "sql_agent":
        logger.info("[Supervisor] Guardrail: plan con tareas pero sin datos. Forzando sql_agent.")
        return {
            "next": "sql_agent",
            "last_agent": "supervisor",
            "iteration_count": next_iter,
            "messages": [AIMessage(content="[Supervisor] Plan listo, ejecutando SQL.")],
        }

    # Si no hay datos, no hay tareas, y no es forecast/research → analyst explicativo
    if not has_data and not plan.tasks and getattr(plan, "question_type") not in ("demand_forecast", "deep_research"):
        logger.info("[Supervisor] Guardrail: plan sin tareas y sin datos. Forzando analyst explicativo.")
        return {
            "next": "analyst",
            "last_agent": "supervisor",
            "iteration_count": next_iter,
            "messages": [AIMessage(content="[Supervisor] Sin tareas ni datos. Redactando explicación.")],
        }
...
Tu trabajo es decidir cuál es el siguiente nodo a ejecutar.

=== AGENTES DISPONIBLES ===
- planner: replanifica.
- sql_agent: ejecuta queries SQL.
...
1. Si no hay plan → planner.
2. Si plan.needs_followup → FINISH.
3. Plan inválido → planner (con feedback).
4. demand_forecast sin forecast_results → forecaster.
5. demand_forecast con resultados/error → analyst.
6. deep_research sin research_findings → researcher.
7. Si hay sql_results con status="needs_clarification" → FINISH.
8. Si hay sql_results con status="unrecoverable" → planner (feedback).
9. Si hay sql_results con status="error" y last_agent != "sql_agent" → sql_agent (feedback).
10. Si no hay sql_results y el plan tiene tareas → sql_agent.
11. visualization_candidate=true, sin viz_result, sin errores → viz_agent.
12. viz_result válido y no renderizado → render_plotly.
13. viz_result inválido/render fallido → analyst.
14. Hay resultados SQL exitosos/partial y no hay final_answer → analyst.
15. Ya existe final_answer → FINISH.

=== REGLAS ABSOLUTAS ===
- NUNCA elijas FINISH si final_answer no está presente.
- NUNCA elijas FINISH si no hay sql_results, forecast_results ni research_findings.
- NUNCA elijas analyst si no se han ejecutado SQL/forecast/research primero.
...
"""
    ...
    # Conversión de dict a SupervisorDecision
    if isinstance(decision_raw, dict):
        logger.info(f"[Supervisor] Decisión recibida como dict: {decision_raw}")
        try:
            decision = SupervisorDecision(**decision_raw)
        except Exception as e:
            logger.error(f"[Supervisor] Error convirtiendo dict: {e}")
            decision = SupervisorDecision(
                reasoning=f"Fallback por conversión inválida: {str(e)}",
                next_agent="analyst",
            )
    elif decision_raw is None:
        decision = SupervisorDecision(
            reasoning="Fallback por error del LLM",
            next_agent="analyst",
        )
    else:
    ...
    if not hasattr(decision, "next_agent") or decision.next_agent not in valid_agents:
        logger.warning(f"[Supervisor] next_agent inválido. Forzando analyst.")
        decision = SupervisorDecision(
            reasoning=f"next_agent inválido corregido a analyst",
            next_agent="analyst",
        )

    # Anti-loop planner
    if decision.next_agent == "planner" and last_agent == "planner":
        logger.warning("[Supervisor] Loop planner→planner. Forzando analyst.")
        decision = SupervisorDecision(
            reasoning="Loop detectado. Se redacta respuesta explicativa.",
            next_agent="analyst",
        )

    # Anti-loop sql_agent sin feedback
    if decision.next_agent == "sql_agent" and last_agent == "sql_agent":
    ...
    # Guardrail absoluto: NUNCA FINISH sin datos
    if decision.next_agent == "FINISH":
        if not final_answer:
            logger.warning("[Supervisor] Guardrail post-LLM: FINISH sin final_answer. Forzando analyst.")
            decision = SupervisorDecision(
                reasoning="FINISH elegido sin respuesta final. Se redacta primero.",
                next_agent="analyst",
            )
        elif not has_data:
    ...
    # 8. Mapear FINISH a __end__
    if decision.next_agent == "FINISH":
        next_node = "__end__"
    else:
        next_node = decision.next_agent

    # 9. Construir update de estado
    update = {
        "next": next_node,
        "last_agent": "supervisor",
        "iteration_count": next_iter,
        ...
        "messages": [
            AIMessage(
                content=f"[Supervisor] Ruta seleccionada: {decision.next_agent} (mapped to {next_node}). "
    ...
    logger.info(f"[Supervisor] Decisión final: {decision.next_agent} (next={next_node}) | {decision.reasoning}")
    return update
